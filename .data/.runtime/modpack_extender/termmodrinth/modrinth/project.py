...
from termmodrinth.utils import sizeof_fmt, convert_isoformat_date, get_file_sha512, sanitize_filename
...
class ModrinthProject(object):
  ...
  # ---------- Filenamen: IMMER sanitisieren (Emoji/Windows-Fixes) ----------

  def storage_filename(self, remote_filename):
    raw = "{}_{}_{}".format(self.slug, self.version_data()['version_number'], remote_filename)
    return sanitize_filename(raw, keep_extension=True, drop_non_bmp=True)

  def storage_filepath(self, remote_filename):
    return "{}/{}".format(self.storage_path, self.storage_filename(remote_filename))

  def info_filename(self):
    raw = "{}_{}.info".format(self.slug, self.version_data()['version_number'])
    return sanitize_filename(raw, keep_extension=True, drop_non_bmp=True)

  def info_filepath(self):
    return "{}/{}".format(self.storage_path, self.info_filename())

  def active_filename(self, is_primary, file_index, extention):
    base = self.slug if is_primary else "{}_{}".format(self.slug, file_index - 1)
    raw = "{}.{}".format(base, extention)
    return sanitize_filename(raw, keep_extension=True, drop_non_bmp=True)

  def active_filepath(self, is_primary, file_index, extention):
    return "{}/{}".format(self.active_path, self.active_filename(is_primary, file_index, extention))
  ...
  def mineInfo(self):
    # WICHTIG: UTF-8 + errors="replace", damit Emojis nie crashen
    with open(self.info_filepath(), 'w', encoding='utf-8', errors='replace') as file_handler:
  ...
  def download(self):
    if len(self.version_data()["files"]):
      if not os.path.isfile(self.info_filepath()):
        ...
        for remote_file in self.version_data()["files"]:
          if self.fileMustBeDownloaded(remote_file["primary"], remote_file["filename"]):
            target_path = self.storage_filepath(remote_file["filename"])
            if not os.path.isfile(target_path):
              ...
              while not self.checkDigest(target_path, remote_file["hashes"]["sha512"]):
                urllib.request.urlretrieve(remote_file["url"], target_path)
            else:
              Logger().projectLog('inf', self.project_type, self.slug, "{} alredy downloaded".format(os.path.basename(target_path)), 'cyan')
      else:
